 ...
 *
   ...
   class InputManager {
   public:
       SubscriptionInterface<InputListener> GetInputSubscriptionInterface() { return m_input_subscription.m_interface; }
 
       void Notify() {
           for (int i = 0; i < m_input_subscription.GetListenerCount(); ++i) {
               m_input_subscription.GetListener(i)->OnEvent();
           }
       }
   private:
       Subscription<InputListener> m_input_subscription;
   };
 *
 *
 * Depends on: 
